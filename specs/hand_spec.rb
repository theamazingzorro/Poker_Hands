...
describe 'Hand' do
  ...
  describe '==' do
    it 'returns true if both hands contain the same 5 cards' do
      hand1 = Hand.new %w[2H 3D 5S 9C KD]
      hand2 = Hand.new %w[2H 3D 5S 9C KD]

      expect(hand1).to eq hand2
    end

    it 'returns true if both hands contain the same 5 cards in different orders' do
      hand1 = Hand.new %w[2H KD 5S 3D 9C]
      hand2 = Hand.new %w[2H 3D 5S 9C KD]

      expect(hand1).to eq hand2
    end

    it 'returns false if both hands contain different cards' do
      hand1 = Hand.new %w[2H KD 5S 3D 9C]
      hand2 = Hand.new %w[2H 3H 5S 9C KD]

      expect(hand1).not_to eq hand2
    end
  end
end